    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    table EDT {
        ...
        actions = {SetClass;}
        size = 95;  // 表大小：95条规则
        default_action = SetClass(1); // 未命中默认为正常流量，转发
    }
  
    apply {
        EDT.apply();
        if (meta.class_id == 0) {
            // class_id=0 表示恶意流量，直接丢弃
            // 规则集中有52条class_id=0的规则用于识别恶意流量
            ig_dprsr_md.drop_ctl = 1;
        } else {
            // class_id=1 表示正常流量，转发到端口0
            // 规则集中有43条class_id=1的规则用于识别正常流量
            // 未匹配规则的包也会走这里（default_action=SetClass(1)）
